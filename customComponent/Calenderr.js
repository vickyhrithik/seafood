...
class Calendarr extends Component {
  ...
  getWeeks = date => {
    const weeks = {
      [parseInt(this.state.pages[0]) - 1]: getWeek(
        moment(date).subtract(2, 'weeks'),
      ),
      ...
      [parseInt(this.state.pages[2]) + 1]: getWeek(
  ...
  handleActiveDayChange = date => {
    const activeWeekNumber = this.state.weeks[this.state.pages[1]][3].format(
      'W',
    );
    ...
    if (
      activeDayWeekNumber - activeWeekNumber === 1 ||
      activeDayWeekNumber - activeWeekNumber === -1
    ) {
    ...
    if (Math.abs(activeDayWeekNumber - activeWeekNumber) > 1) {
      ...
      this.setState({
        weeks: weeks,
        activeDay: this.props.activeDay,
        showingMonths: getShowingHeaderItems(
          weeks[this.state.pages[1]],
          'MMMM',
        ),
        showingYears: getShowingHeaderItems(weeks[this.state.pages[1]], 'YYYY'),
      });
  ...
  swiperScrollHandler = index => {
    ...
    if (index === 0) {
      ...
      const weeks = {
        ...this.state.weeks,
        [nextKey]: getWeek(
          moment(this.state.weeks[this.state.pages[1]][3]).subtract(3, 'weeks'),
        ),
      };
      ...
      const weeks = {
        ...this.state.weeks,
        [nextKey]: getWeek(
          moment(this.state.weeks[this.state.pages[1]][3]).add(3, 'weeks'),
        ),
      };
...
const Day = props => {
  return (
    <TouchableOpacity
      ...
      style={[
        styles.Day,
        props.active ? {borderBottomColor: props.activeDayBorderColor} : null,
      ]}>
      <Text
        style={[{color: 'steelblue', fontSize: 12}, {...props.dateNameStyle}]}>
        {props.dayInWeekName}
      </Text>
      <Text style={[{fontSize: 15}, {...props.dateNumberStyle}]}>
        {props.dayNumber}
      </Text>
    </TouchableOpacity>
  );
